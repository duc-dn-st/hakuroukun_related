...
class HakuroukunPose:
    ...
    def _get_xy_from_lat_lon(self, lat,lon):
        ...
        # rotation matrix was calculate manually (-90 - arctan(0.6/5))
        rotation_angle = math.radians(-90 -     6.84)
        self.current_pose_x = x*math.cos(rotation_angle) - y*math.sin(rotation_angle)
        self.current_pose_y = x*math.sin(rotation_angle) + y*math.cos(rotation_angle)
